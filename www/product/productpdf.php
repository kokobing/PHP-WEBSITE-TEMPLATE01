...
$strSQL = "select distinct(id_prodinfo) from productpic  where type='PDF'  order by id_prodpic  desc" ;
$objDB->Execute($strSQL);
$arrproductpdf=$objDB->GetRows();

   $arrproductpdfid='';
   for($i=0;$i<sizeof($arrproductpdf);$i++){
      if($i==sizeof($arrproductpdf)-1){
	  $arrproductpdfid.=$arrproductpdf[$i][id_prodinfo];
	  }else{
      $arrproductpdfid.=$arrproductpdf[$i][id_prodinfo].",";
	  }
   }//组合有存在PDF的不重复的产品ID
...
$intRows = 10;
//产品
$strSQLNum = "SELECT COUNT(*) as num from productinfo  where  dele=1 and id_prodinfo in ($arrproductpdfid)";   
...
$strSQL = "select * from productinfo  where dele=1 and id_prodinfo in ($arrproductpdfid) order by id_prodinfo desc" ;
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="txt">
  <tr>
    <td height="30" align="left" class="txt_01"> 首页 &gt;产品中心 &gt; <span class="txt_02">资料下载</span></td>
  </tr>
  <tr>
    <td height="120"><img src="../inc/pics/pdfdownload.gif" width="355" height="76" /></td>
  </tr>
</table>
</div>
