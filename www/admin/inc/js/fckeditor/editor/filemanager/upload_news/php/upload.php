...
while ( true )
{
	// Compose the file path.
			    $filename = (substr(time(),1,10)+ mt_rand(1,10)). mt_rand(1,10) . '.'.$sExtension;
				$sFilePath = $sServerDir . $filename ;
	...
	else
	{
		...
		if ( $Config['UseFileType'] )
			$sFileUrl = $Config["UserFilesPath"] . $sType . '/' . $filename ;
		else
			$sFileUrl = $Config["UserFilesPath"] . $filename ;

		break ;
	}
}

SendResults( $sErrorNumber, $sFileUrl, $filename ) ;
?>
