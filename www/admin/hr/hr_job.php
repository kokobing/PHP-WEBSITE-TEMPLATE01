...
$intRows = 10;
$strSQLNum = "SELECT COUNT(*) as num from newsinfo left join newsdir on newsinfo.id_newsdir=newsdir.id_newsdir WHERE newsinfo.id_newsdir='7' and newsinfo.dele='1'";   
...
$strSQL = "SELECT newsinfo.id_newsinfo,newsdir.name,newsinfo.newsdate,newsinfo.title,newsinfo.content FROM newsinfo left join newsdir on newsinfo.id_newsdir=newsdir.id_newsdir WHERE newsinfo.id_newsdir='7' and newsinfo.dele='1' order by newsinfo.indate DESC" ;
...
if(isset($action) && $action=="02" && $onuserperm_addprem=='1'){
	...
	$strSQL="INSERT INTO newsinfo(title,content,id_newsdir,url,newsdate,indate) 
	                     values('$newstitle','$newscontent','7','$url','$fbtime',now())";
	$objDB->Execute($strSQL);
    header('Location:hr_job.php'); exit();
}	
//编辑新闻入库
if(isset($action) && substr($action,0,2)=="04" && $onuserperm_editprem=='1'){
    $onenewsinfoid=substr($action,2);
	echo $onenewsinfoid;
    $strSQL="UPDATE newsinfo SET title='$newstitle',content='$newscontent',id_newsdir='7',url='$url',newsdate='$fbtime',modate=now() where id_newsinfo=$onenewsinfoid";
	$objDB->Execute($strSQL);
	header('Location:hr_job.php'); exit();
}	
